# Python-Projects
A collection of Python projects covering programming fundamentals, problem-solving, data analysis, machine learning, and AI
##  Projects

| # | Project | Concepts Used |
|---|---------|---------------|
| 1 | [Student Grade Calculator](./01_Student_Grade_Calculator) | Variables, Input, Conditions, Functions |
| 2 | [Number Guessing Game](./02_Number_Guessing_Game) | Loops, Conditions, Random Module, Input |
| 3 | [Expense Tracker](./03_Expense_Tracker) | Lists, Functions, Loops, Conditions |
| 4 | [Library Management System](./04_Library_Management_System) | Lists, Functions, Loops, Conditions, String Operations |

---

##  Skills Practiced

- Python fundamentals
- Variables and data types
- User input and output
- Conditional statements
- for loops
- while loops
- Functions
- Lists
- String operations
- Basic problem solving
---
## Beginner Python
- [x] Student Grade Calculator
- [x] Number Guessing Game
- [x] Expense Tracker
- [x] Library Management System
## Technologies used
- Python3
- vs code
- git
- github
  
